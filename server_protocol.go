...
func (p *Display) Dispatch(event *Event) {
	...
	case 0:
		if len(p.syncHandlers) > 0 {
			ev := DisplaySyncEvent{}
			ev.Callback = NewCallback(p.Context())
			p.mu.RLock()
	...
	case 1:
		if len(p.getRegistryHandlers) > 0 {
			...
			ev.Registry = NewRegistry(p.Context())
			p.mu.RLock()
...
func (p *DataDevice) Dispatch(event *Event) {
	...
	case 0:
		if len(p.startDragHandlers) > 0 {
			...
			ev.Source = event.Proxy(p.Context()).(*DataSource)
			ev.Origin = event.Proxy(p.Context()).(*Surface)
			ev.Icon = event.Proxy(p.Context()).(*Surface)
			ev.Serial = event.Uint32()
			p.mu.RLock()
...
func (p *Shell) Dispatch(event *Event) {
	...
	case 0:
		if len(p.getShellSurfaceHandlers) > 0 {
			...
			ev.Id = NewShellSurface(p.Context())
...
func (p *Surface) Dispatch(event *Event) {
	...
	case 1:
		if len(p.attachHandlers) > 0 {
			...
			ev.Buffer = event.Proxy(p.Context()).(*Buffer)
			ev.X = event.Int32()
			ev.Y = event.Int32()
			p.mu.RLock()
	...
	case 4:
		if len(p.setOpaqueRegionHandlers) > 0 {
			...
			ev.Region = event.Proxy(p.Context()).(*Region)
			p.mu.RLock()
