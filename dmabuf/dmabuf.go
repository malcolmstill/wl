	...
	case 0:
		if len(p.destroyHandlers) > 0 {
			...
			for _, h := range p.destroyHandlers {
				h.Handlewl.ZwpLinuxDmabufV1Destroy(ev)
			}
			p.mu.RUnlock()
		}
	case 1:
		if len(p.createParamsHandlers) > 0 {
			...
			for _, h := range p.createParamsHandlers {
				h.Handlewl.ZwpLinuxDmabufV1CreateParams(ev)
			}
			p.mu.RUnlock()
		}
	}
...
func (p *ZwpLinuxBufferParamsV1) Dispatch(event *wl.Event) {
	...
	case 0:
		if len(p.destroyHandlers) > 0 {
			...
			p.mu.RLock()
			for _, h := range p.destroyHandlers {
				h.Handlewl.ZwpLinuxBufferParamsV1Destroy(ev)
			}
			p.mu.RUnlock()
		}
	case 1:
		if len(p.addHandlers) > 0 {
			...
			for _, h := range p.addHandlers {
				h.Handlewl.ZwpLinuxBufferParamsV1Add(ev)
			}
			p.mu.RUnlock()
		}
	case 2:
		if len(p.createHandlers) > 0 {
			...
			for _, h := range p.createHandlers {
				h.Handlewl.ZwpLinuxBufferParamsV1Create(ev)
			}
			p.mu.RUnlock()
		}
	case 3:
		if len(p.createImmedHandlers) > 0 {
			...
			ev.BufferId = Newwl.Buffer(p.Context(), int(event.Uint32()))
			ev.Width = event.Int32()
			ev.Height = event.Int32()
			ev.Format = event.Uint32()
			ev.Flags = event.Uint32()
			...
			for _, h := range p.createImmedHandlers {
				h.Handlewl.ZwpLinuxBufferParamsV1CreateImmed(ev)
			}
			p.mu.RUnlock()
		}
	}
...
//
func (p *ZwpLinuxBufferParamsV1) Created() (*wl.Buffer, error) {
	ret := Newwl.Buffer(p.Context())
